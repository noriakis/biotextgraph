...
The `enrich` option can be specified, this time `'reactome'`.

```{r ngrampath, warning=FALSE, fig.height=6, fig.width=6}
gwc3 <- wcGeneSummary(inpSymbol, enrich="reactome", tfidf=TRUE, numWords=50)
gwc3$wc
```

One of the main questions is which words can be clustered together among the words contained in the queried gene cluster. Word clustering (`pvclust`) and identified significant clusters based on the occurrence in the text can be visualized by specifying `tag=TRUE`. For the significance threshold, `pvclAlpha` can be specified. The default parameters perform `pvclust` on the subset of dataset for words with high frequency specified by `numWords`. If one want to perform on whole matrix, `tagWhole=TRUE` can be specified, although it is computationally intensive.
...
```{r combineapp, cache=T, fig.height=10, fig.width=10}
