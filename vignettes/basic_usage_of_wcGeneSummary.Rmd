...
```{r gplot, warning=FALSE, message=FALSE, cache=T, fig.height=8, fig.width=8}
...
```{r gpplot, warning=FALSE, message=FALSE, cache=T, fig.width=8, fig.height=8}
net <- wcGeneSummary(inpSymbol, plotType="network", genePathPlot="reactome", corThresh=0.5,
...
Using `easyPubMed`, one can perform the same analysis on PubMed text like the article title and abstract. The function queries for the input gene symbols (or microbes) and visualize. For typical use cases, the genes identified by showing `genePlot`, or hub genes identified in gene network analysis can be queried.

```{r wcabst_wc, cache=TRUE, fig.height=8, fig.width=8}
...
```{r wcabst_wc_2, cache=T, fig.height=8, fig.width=8}
...
```{r compare, cache=T, fig.height=8, fig.width=8}
cxcls <- c()
...
```{r tagcomp, cache=F, fig.height=10, fig.width=8}
...
```{r wgcna, warning=FALSE, message=FALSE, cache=T, fig.width=6, fig.height=9}
...
frq <- findTerm(query, listOfGenes[sample(length(listOfGenes), 20)],
                split=TRUE,
                keyType="ENTREZID")
plt <- data.frame(t(data.frame(frq, check.names=F)), check.names=FALSE)
