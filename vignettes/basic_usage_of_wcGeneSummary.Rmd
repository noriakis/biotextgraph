...
This package aims to visualize the word and text information contained in the gene or the other omics identifiers such as microbiome, and compare the clusters based on those information.
...
As many of the words are commonly observed, you should limit word frequency by `excludeFreq`, which is default to 2000. TF-IDF on the all the summary is precomputed, and `excludeTfIdf` can be specified too.
...
The function accepts the other parameters which is passed to `wordcloud()`.
...
Word clustering (pvclust) based on the occurrence in the text can be visualized by specifying `tag=TRUE`.
...
net <- wcGeneSummary(inpSymbol, plotType="network", edgeLabel=TRUE, corThresh=0.5, numWords=20)
net$net
```

The clustering results based on `pvclust` can be shown also on the correlation network, by the node color.

```{r tagnet, fig.cap="A correlation network of genes with cluster information", warning=FALSE, message=FALSE, cache=T}
...
The genes associated with the corresponding words can be shown by specifying `genePlot=TRUE`.
The edges connecting word to corresponding gene are shown.

```{r gplot, fig.cap="A correlation network of gene summaries with associated genes", warning=FALSE, message=FALSE, cache=T}
...
```{r gpplot, fig.cap="A correlation network of gene summaries with associated genes", warning=FALSE, message=FALSE, cache=T}
...
compareWordNet(net1, net2, titles=c("ercc","cxcl"))
```
...
compareWordNet(net1, net2, tag=TRUE)
```
...
Using `easyPubMed`, one can perform the same analysis on PubMed text like the article title and abstract. The function recursively queries for the input gene symbols (or microbes), concatenates the results and visualize.
...
As fetching the same information is time consuming, The same object can be passed to `redo` option and reperform the analysis like tagging.

```{r wcabst_wc_2, cache=F}
